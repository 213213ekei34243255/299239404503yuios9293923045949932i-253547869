    ...
    webview.addEventListener("new-window", (e) => {
        ...
        if (url.includes("accounts.google.com")) {
            if (window.api?.openExternal) {
                window.api.sendOpenExternal(url);
            } else {
                console.error("❌ openExternal not ready");
            }
        }
    });
...
function attachErrorHandlers(webview) {
    ...
    webview.addEventListener("did-fail-load", (e) => {
        ...
        if (
            e.errorDescription &&
            e.errorDescription.toLowerCase().includes("internet")
        ) {
            type = "offline";
        }

        const errorPage =
            `file://${location.pathname.replace("index.html", "")}error.html?type=${type}`;
